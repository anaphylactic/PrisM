		...
		try {
			...
			try{
				...
				for (long multi = iter.open() ; multi != 0 ; multi = iter.getNext()){
					...
					String t2 = iter.getGroundTerm(1).toString();
					String t3 = iter.getGroundTerm(2).toString();
					if (Utility_tme.removeAngleBrackets(t2).equals(Namespace.RDF_TYPE)){
						OWLClass c = factory.getOWLClass(IRI.create(Utility_tme.removeAngleBrackets(t3)));
						OWLIndividual i = factory.getOWLNamedIndividual(IRI.create(Utility_tme.removeAngleBrackets(t1)));
						manager.addAxiom(o, factory.getOWLClassAssertionAxiom(c, i));
					}
					else if (Utility_tme.removeAngleBrackets(t2).equals(Namespace.EQUALITY)){
						OWLIndividual i = factory.getOWLNamedIndividual(IRI.create(Utility_tme.removeAngleBrackets(t1)));
						OWLIndividual j = factory.getOWLNamedIndividual(IRI.create(Utility_tme.removeAngleBrackets(t3)));
						System.out.println("Equality Assertions in the dataset..." + t1+t2+t3);
						manager.addAxiom(o, factory.getOWLSameIndividualAxiom(i,j));
					}
					else if (Utility_tme.removeAngleBrackets(t2).equals(Namespace.INEQUALITY)){
						OWLIndividual i = factory.getOWLNamedIndividual(IRI.create(Utility_tme.removeAngleBrackets(t1)));
						OWLIndividual j = factory.getOWLNamedIndividual(IRI.create(Utility_tme.removeAngleBrackets(t3)));
						...
						OWLIndividual i = factory.getOWLNamedIndividual(IRI.create(Utility_tme.removeAngleBrackets(t1)));
						OWLIndividual j = factory.getOWLNamedIndividual(IRI.create(Utility_tme.removeAngleBrackets(t3)));
						OWLObjectProperty r = factory.getOWLObjectProperty(IRI.create(Utility_tme.removeAngleBrackets(t2)));
